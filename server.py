...
<header class="top shell"><a class="brand" href="/"><span class="mark">CS</span><span>CS101 题库</span></a><nav class="nav"><a href="/problems/">全部题目</a><a href="/history/">提交记录</a><span class="user-menu" id="user-menu"><a id="account" href="/auth/login/">登录</a><span class="account-menu"><span class="account-menu-inner"><a href="/account/">账户设置</a><button id="logout" type="button">退出登录</button></span></span></span><a class="primary" href="/{escape(book)}/{escape(problem)}/submit/">提交代码</a></nav></header>
<main class="shell"><div class="crumb"><a href="/">首页</a> / <a href="/problems/">题库目录</a> / {escape(book)} / {escape(problem)}</div><div class="layout"><article class="article"><div class="eyebrow">Problem statement</div><h1>{escape(title)}</h1>{params_html}<dl class="problem-content">{content_html}</dl></article><aside class="aside"><h2>题目概览</h2><dl>{stats_html}</dl><div class="aside-note">本题使用测试数据判题。<br><a href="/{escape(book)}/{escape(problem)}/submit/">打开提交页 →</a></div></aside></div></main><footer class="footer shell">CS101 · 题面与判题服务</footer><script>fetch('/api/me').then(r=>r.json()).then(d=>{{const menu=document.querySelector('#user-menu');if(d.authenticated){{menu.classList.add('authenticated');account.textContent=d.user;account.href='/history/';}}}});document.querySelector('#logout').addEventListener('click',async()=>{{await fetch('/api/logout',{{method:'POST',credentials:'same-origin'}});location.reload();}});</script></body></html>"""
