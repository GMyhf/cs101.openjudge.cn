#!/usr/bin/env python3
"""Fetch the already-accepted round17 sources; credentials stay in env vars."""
import html
import json
import os
import re
...
def main():
    ...
    login = opener.open(urllib.request.Request("http://cs101.openjudge.cn/api/auth/login/", data=body), timeout=60)
    ...
    for entry in json.loads(MANIFEST.read_text())["entries"]:
        n = int(entry["local_number"]); accepted = entry["existing_accepted"]
        ...
        (ROOT / f"scripts/t004_platform_accepted_{n}{suffix}").write_text(header + source)
        print(n, suffix, accepted["solution_id"])
