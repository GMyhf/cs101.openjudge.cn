...
给定只包含小写字母的字符串 A, B ，以及 k 个禁止出现的长度为 2 的字符串（称为禁用组合）。

构造字符串 C 的方式如下：

1. C 起始是空串；
2. 按顺序枚举 A 的每个字符 Ai，依次与 B 的每个字符 Bj 组合成串 AiBj；
3. 若 AiBj 不在禁用组合中，则将该串按加入到 C 的末尾。

给定 q 个询问，每个询问给出 ti，求 C 的第 ti 个字符，保证所有询问都不会越界。

**输入**

第一行包含四个整数 LA, LB, k, q, 其中 LA, LB 分别为 A, B 的长度..
第二行包含 LA 个小写字符, 表示字符串 A.
第三行包含 LB 个小写字符, 表示字符串 B.
接下来 k 行每行有两个小写字符, 用一个空格分开, 表示 k 组禁用组合.
接下来 q 行每行一个正整数, 第 i 个表示第 i 次询问对应的 ti. 注意字符串的下标从 0 开始. 记 C 的长度为 LC, 保证 0 <= ti < LC.

**输出**

共 q 行, 每行一个字符, 第 i 行的表示第 i 次询问的答案.
...
对所有数据, LA, LB <= 200000, k <= 26^2, q <= 100000 .
对 60% 的数据: LA, LB <= 1000.
对另外 20% 的数据: q <= 10.
