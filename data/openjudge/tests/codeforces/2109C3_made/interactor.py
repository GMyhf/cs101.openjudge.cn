"""Interactor for Codeforces 2109C3 "Hacking Numbers (Hard Version)": at most f(n) commands per test.
...
def limit_for(n):
    # f(n) = 2 for n = 81 ("mul 999999999", "digit": S(999999999 * x) = 81 for 1 <= x <= 1e9),
    # f(n) = 3 otherwise (add n - 81 afterwards).  Two commands cannot reach any other n: one of
    # them would have to map all 1e9 values of x to at most two values, and only "mul y" followed
    # by "digit" can; S(x*y) = S(y) for every x forces y = 0 mod 9, and an exhaustive check over
    # y <= 1e9 (larger y overflow for x = 1e9 and leave S(1e9) = 1) finds only y = 999999999.
