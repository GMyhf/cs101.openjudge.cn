#!/usr/bin/env python3
"""Codeforces 2227E It All Went Sideways -- generator, input contract, oracle and build.
...
Output the maximum number of cubes whose column changes.
...
Shapes (seed -> how solutions go wrong):
  ...
  8   plateau traps: several runs where the suffix minimum is flat; the best run is
      not at the end and not at the global minimum -- kills "decrease a_n" and
      "decrease the first global minimum" greedies.
  ...
  14  n=2e5 strictly decreasing (huge answer, sum ~ 2e10: 32-bit overflow).
  15  n=2e5: a=[n]*(n-1)+[1] (every tall cube moves; decrement choice irrelevant/overflow).
  16  n=2e5 plateau trap: big best run far from the end.
...
physically slides cubes one cell at a time until nothing moves, and counts cubes
whose column changed.  It is run on every file whose cost estimate fits the budget.
"""
...
def generate(seed, attempt=0):
    r = random.Random(2227_0005_000 + seed * 7919 + attempt)
    if seed == 1: tests = [[1]]
    elif seed == 2:
...
def metric(a):
    return sum(1 for (i, h), c in settle(a).items() if c != i)
